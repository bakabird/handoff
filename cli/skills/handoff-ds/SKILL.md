...
```bash
handoff new --backend deepseek --slug <≤3个英文单词的任务助记词> --write <<'__HF_EOF__'
[prompt 内容]
__HF_EOF__
```
...
```
/Users/x/.handoff/tasks/0720-ds-06-fix-auth.prompt.md
```

去掉目录和 `.prompt.md` 后缀，剩下的 `0720-ds-06-fix-auth` 就是本次任务的 **RUN_ID**。
...
```bash
handoff run --backend deepseek ~/.handoff/tasks/<RUN_ID>.prompt.md
```
...
handoff new --backend deepseek --slug <任务助记词> --write <<'__HF_EOF__'
[后续任务内容]
__HF_EOF__

# 第 2 步：run_in_background: true
handoff resume <首次RUN_ID> --backend deepseek ~/.handoff/tasks/<新RUN_ID>.prompt.md
```
