---
name: handoff-gemini
description: 把执行性编码/调查任务整包交给 gemini 后台执行，省主会话额度。后台运行，完成后自动通知。支持并行多任务，支持续接（resume）上次会话继续派发后续任务。
---

# handoff-gemini Skill
...
```bash
handoff run --backend gemini --slug <≤3个英文单词的任务助记词> - <<'__HF_EOF__'
[prompt 内容]
__HF_EOF__
```
...
启动后从 **stdout/stderr** 捕获 `RUN_ID=<id>`（如 `RUN_ID=0613-ge-03-fix-auth`）；这就是本次任务的 run_id。记住它，用户要求"继续上次/接着再做 X"时靠它 `resume`。

命令进程退出后，从 **stdout/stderr** 捕获唯一完成标记 `RESULT=<任务路径>`（如 `~/.handoff/tasks/0613-ge-03-fix-auth.result.md`），缩写成 `~/.handoff/...` 后回显给用户。`RESULT=` 只表示任务已结束且结果文件已落盘。
...
```bash
handoff resume <run_id> --backend gemini --slug <任务助记词> - <<'__HF_EOF__'
[后续任务内容]
__HF_EOF__
```
